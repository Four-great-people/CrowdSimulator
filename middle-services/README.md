...
```
### POST /maps/\<id\>/statistics — запустить расчёт маршрутов
- Достаёт карту из БД,
...
- считает один шаг за 10 секунд, диагональ - 15 секунд, возвращает ответ в секундах
- null - отсутствие маршрута
```bash
curl -X POST http://127.0.0.1:5000/maps/<id>/statistics
```
```
{
    "ideal": null,
